...
describe('Loja — Pedidos (STO.9)', () => {
  it('renders the admin-04 board: buyer · number, item summary and status chips', async () => {
    ...
    expect(await screen.findByText('Lucas Almeida · #2431')).toBeInTheDocument();
    expect(screen.getByText('Kimono oficial · A2 · R$ 389 · Pix')).toBeInTheDocument();
    expect(screen.getByText('Recebido')).toBeInTheDocument();

    expect(screen.getByText('Fernanda Silveira · #2430')).toBeInTheDocument();
    expect(screen.getByText('Camiseta da equipe · P · R$ 69 · Pix')).toBeInTheDocument();
    expect(screen.getByText('Em andamento')).toBeInTheDocument();

    expect(screen.getByText('Marina Costa · #2428')).toBeInTheDocument();
    expect(screen.getByText('Rash guard · M · R$ 149 · Pix')).toBeInTheDocument();

    // Sizeless item skips the tam segment.
    expect(screen.getByText('João Ferraz · #2425')).toBeInTheDocument();
    expect(screen.getByText('Protetor bucal · R$ 39 · Pix')).toBeInTheDocument();
      ...
      ...adminStoreHandlers({
        orders: [
          ...
          makeAdminStoreOrder({
            number: 2440,
            status: 'pending',
            buyer: { userId: '018f0000-0000-7000-8054-00000000ffff', fullName: 'Pedro Pendente' },
          }),
        ],
      }),
    );
  ...
  it('opens the admin-05 sheet: item card, retirada note, options and the atual marker', async () => {
    ...
    await user.click(await screen.findByRole('button', { name: /Lucas Almeida · #2431/ }));
    const sheet = await screen.findByRole('dialog', { name: 'Pedido #2431' });

    expect(within(sheet).getByText('Lucas Almeida · pago via Pix')).toBeInTheDocument();
    ...
    expect(within(sheet).getByText('Pago — aguardando separação')).toBeInTheDocument();
    expect(within(sheet).getByText('Separando o item para retirada')).toBeInTheDocument();
    expect(
    ...
    expect(within(sheet).getByRole('button', { name: 'Recebido' })).toBeDisabled();
    // Valid transitions from paid: ready and canceled only — no skipping.
    expect(within(sheet).getByRole('button', { name: 'Em andamento' })).toBeEnabled();
    expect(within(sheet).getByRole('button', { name: 'Entregue' })).toBeDisabled();
    expect(within(sheet).getByRole('button', { name: 'Cancelado' })).toBeEnabled();
  });
  ...
  it('advances a paid order to Em andamento with the notified toast', async () => {
    ...
    server.use(
      http.post('/v1/admin/store/orders/{id}/status', async ({ params, request, response }) => {
        posted = { id: params.id, body: (await request.json()) as Record<string, unknown> };
    ...
    await user.click(await screen.findByRole('button', { name: /Lucas Almeida · #2431/ }));
    const sheet = await screen.findByRole('dialog', { name: 'Pedido #2431' });
    await user.click(within(sheet).getByRole('button', { name: 'Em andamento' }));

    expect(
      await screen.findByText('Pedido #2431 marcado como em andamento — comprador notificado.'),
    ).toBeInTheDocument();
  ...
  it('advances a ready order to Entregue; delivered is then terminal', async () => {
    ...
    expect(
      await screen.findByText('Pedido #2430 marcado como entregue — comprador notificado.'),
    ).toBeInTheDocument();
  ...
  it('freezes terminal orders: a delivered pedido offers no transition', async () => {
    ...
    await user.click(await screen.findByRole('button', { name: /Marina Costa · #2428/ }));
    const sheet = await screen.findByRole('dialog', { name: 'Pedido #2428' });

    expect(within(sheet).getByRole('button', { name: 'Recebido' })).toBeDisabled();
    expect(within(sheet).getByRole('button', { name: 'Em andamento' })).toBeDisabled();
    expect(within(sheet).getByRole('button', { name: 'Entregue' })).toBeDisabled();
    expect(within(sheet).getByRole('button', { name: 'Cancelado' })).toBeDisabled();
    expect(within(sheet).getByText('atual')).toBeInTheDocument();
  });
  ...
  it('cancels only after the estorno confirm, with the refund toast', async () => {
    ...
    const confirm = await screen.findByRole('dialog', { name: 'Cancelar pedido' });
    expect(
    ...
    expect(
      await screen.findByText('Pedido #2431 marcado como cancelado — estorno iniciado.'),
    ).toBeInTheDocument();
  ...
  it('maps store.order_invalid_transition to the PT-BR message', async () => {
    ...
    server.use(
      http.post('/v1/admin/store/orders/{id}/status', ({ response }) =>
        response.untyped(problemResponse(409, 'store.order_invalid_transition')),
      ),
    );
    const user = userEvent.setup();
    await openPedidos(user);
  ...
  it('uses makeStoreOrderItem defaults for ad-hoc orders', async () => {
      ...
      ...adminStoreHandlers({
        orders: [
          makeAdminStoreOrder({
            ...
            status: 'paid',
            totalCents: 77_800,
            buyer: { userId: '018f0000-0000-7000-8054-00000000aaaa', fullName: 'Bia Andrade' },
    ...
    expect(screen.getByText('Kimono oficial · A2 · R$ 778 · Pix')).toBeInTheDocument();
  });
});
