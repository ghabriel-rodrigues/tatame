...
describe('registry belt chips (GRD.14)', () => {
  it('renders derived belt subtitles on student rows per admin-07', async () => {
    ...
    expect(
      await screen.findByText('Faixa azul · 2 graus · Fundamentos'),
    ).toBeInTheDocument();
    expect(
      screen.getByText('Faixa roxa · 1 grau · Avançada'),
    ).toBeInTheDocument();
...
describe('initial-belt select on the aluno form (GRD.14)', () => {
  it('offers enabled belts only and posts initialBeltId', async () => {
    ...
    expect(
      screen.queryByRole('option', { name: 'Faixa laranja' }),
...
describe('graduation-history drawer + Revogar (GRD.14)', () => {
  ...
  it('renders the immutable timeline with revocation rows distinct', async () => {
    ...
    expect(
      within(drawer).getByText('Promoção · Faixa azul'),
    ).toBeInTheDocument();
    expect(
      within(drawer).getByText('1º grau · Faixa azul'),
    ).toBeInTheDocument();
    expect(
      within(drawer).getByText('2º grau · Faixa azul'),
    ).toBeInTheDocument();
    ...
    expect(
      within(drawer).getByText(/15\/01\/2026 · por Rafael Nunes/),
    ).toBeInTheDocument();
    expect(
      within(drawer).getByText(/por Amanda Admin · Lançamento incorreto\./),
    ).toBeInTheDocument();
    // Revogar only on non-reversed awards: promotion + 1º grau.
    expect(
      within(drawer).getAllByRole('button', { name: 'Revogar' }),
    ).toHaveLength(2);
  });
  ...
  it('revokes an award after confirmation and posts the audited reason', async () => {
    ...
    await user.click(
      within(drawer).getAllByRole('button', { name: 'Revogar' })[0]!,
    );
    const confirm = await screen.findByRole('dialog', {
      name: 'Revogar graduação',
    });
    await user.type(
    ...
    expect(revoked!.body).toMatchObject({
      reason: 'Grau lançado para o aluno errado.',
    });
  });
  ...
  it('maps graduation.already_reversed to PT-BR (single reversal rule)', async () => {
    ...
    expect(
      await screen.findByText('Esta graduação já foi revogada.'),
    ).toBeInTheDocument();
  });
});
