...
import { ApiErrorCodes, isProblemCode, type AttentionRow } from '@tatame/shared';
...
function StatTile({ value, caption, tone }: { value: string; caption: string; tone?: 'danger' }) {
  return (
        ...
        {data ? (
          <>
            ...
            <Stack direction="row" spacing="12px">
              <StatTile value={formatCount(data.academyCount)} caption="academias" />
              <StatTile value={formatCount(data.studentCount)} caption="alunos na base" />
              <StatTile
            ...
            <Card padding={16}>
              <Typography
                sx={{ fontSize: 14.5, fontWeight: 700, color: 'var(--fg-1)', marginBottom: '10px' }}
              >
                MRR · últimos 6 meses
              </Typography>
              <MiniBarChart
            ...
            <Card padding={4}>
              {data.attention.length === 0 ? (
                ...
                data.attention.map((row) => (
                  <AttentionListRow key={`${row.academyId}-${row.kind}`} row={row} />
                ))
              )}
            </Card>
          </>
