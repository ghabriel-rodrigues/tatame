...
import { academyPlans, withTenant, type DbHandle, type DbTransaction } from '@tatame/db';
...
export class PlansService {
  ...
  async list(ctx: AuthContext & { tenantId: string }): Promise<PlanHeaderView[]> {
    return withTenant(this.appDb.db, { tenantId: ctx.tenantId, userId: ctx.userId }, async (tx) => {
      const rows = await tx.select().from(academyPlans).orderBy(asc(academyPlans.name));
      return rows.map(view);
    });
  }

  async create(ctx: AuthContext & { tenantId: string }, input: PlanInput): Promise<PlanHeaderView> {
    return withTenant(this.appDb.db, { tenantId: ctx.tenantId, userId: ctx.userId }, async (tx) => {
      ...
      const [row] = await tx
        ...
        .returning();
      if (!row) throw problem(500, ErrorCodes.INTERNAL, 'Plan insert returned no row');
      return view(row);
    });
  }
  ...
  async update(
    ...
    return withTenant(this.appDb.db, { tenantId: ctx.tenantId, userId: ctx.userId }, async (tx) => {
      ...
      const [row] = await tx
        ...
        .returning();
      if (!row) throw problem(404, ErrorCodes.PLAN_NOT_FOUND, 'Plan not found');
      return view(row);
    });
  }

  /** Soft archive: refuses new assignment, keeps every reference intact. */
  async archive(ctx: AuthContext & { tenantId: string }, id: string): Promise<PlanHeaderView> {
    return withTenant(this.appDb.db, { tenantId: ctx.tenantId, userId: ctx.userId }, async (tx) => {
      const [row] = await tx
        .update(academyPlans)
  ...
  private async assertNameFree(tx: DbTransaction, name: string, exceptId?: string): Promise<void> {
    const [existing] = await tx
      .select({ id: academyPlans.id })
      .from(academyPlans)
      .where(
        and(eq(academyPlans.name, name), exceptId ? ne(academyPlans.id, exceptId) : undefined),
      );
    if (existing) {
      throw problem(409, ErrorCodes.PLAN_NAME_TAKEN, 'A plan with this name already exists');
    }
  }
}
