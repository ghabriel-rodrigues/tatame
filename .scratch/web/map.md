...
- Auth session: in-memory access token (singleton auth store) + httpOnly `SameSite=Lax` refresh cookie on same-origin `/api/auth` (bearer header on API calls, shared with mobile); silent refresh on boot returns `{accessToken, session{user, memberships, impersonation}}`; single-flight 401 refresh fills ticket 03's adapter seam; guards read bootstrapped store; post-login redirect `?next` > last-used > single surface > both→`/plataforma`, mobile-only personas→`/baixe-o-app`; impersonation bound server-side to refresh session (`act` claim, audit, banner, `queryClient.clear()` both ways); logout = refresh revocation + clear; reset ends at `/login` (no auto-login); Convite signup ends logged-in via same redirect rule. Backend contract points marked *(assumption)* pending `.scratch/backend/issues/02` — [issues/04-auth-session-web.md](issues/04-auth-session-web.md)
- Testing: Vitest + RTL (ticket 01 scaffold) with route/page-level tests as the bulk seam — real router + providers + MSW at the fetch layer (`openapi-msw` typed from the committed schema.d.ts; shared handlers in `packages/shared/src/api/testing/`), never stub the client/hooks; mandatory RBAC guard suite per surface; Playwright thin e2e smoke (`apps/web-e2e`, ~6 specs: login, silent refresh, guard redirects, logout, invalid Convite) against built app + docker-compose API, no MSW in e2e; no screenshot-diff CI (human review vs handoff + token lint); CI = GitHub Actions nx-affected *(assumption pending infra 03)*, Netlify never runs tests — [issues/07-testing-strategy.md](issues/07-testing-strategy.md)
...
- Client-side state beyond server state — auth/tenant/impersonation settled by ticket 04 (singleton auth store; tenant + impersonation are server-derived session facts); remaining: theme/white-label palette state, owned by the design-system integration (ticket 05).
