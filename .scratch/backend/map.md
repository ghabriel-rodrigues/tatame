...
- Realtime live attendance: **SSE via Nest `@Sse()` (`GET /v1/professor/live-codes/:id/stream?ticket=`) — read-side only, WS rejected (no bidirectional need; check-ins stay REST POST); auth = ~60 s single-purpose HMAC ticket minted by a bearer-auth REST endpoint (native EventSource on web, `react-native-sse`/`okhttp-sse`/`URLSession.bytes` on natives); documented OpenAPI exception with payload as component schema; snapshot-then-stream, heartbeat 20 s, 5 s foreground polling fallback; in-process room Subjects fed by `attendance.checkin.recorded`; notification badges explicitly NOT on this channel** — [issues/09-realtime-live-attendance.md](issues/09-realtime-live-attendance.md)
- Payments architecture: **`PaymentProviderPort` in `infra/payments/` (Resend-seam pattern; billing module is sole consumer, normalized `ProviderEvent` union so simulated and Stripe paths share one handler): v1 `SimulatedPaymentProvider` (deterministic Pix QR/copia-e-cola + boleto linha digitável in `payments.provider_data`, settlement via `POST /v1/billing/payments/:id/simulate` honoring the handoff button, 404 unless `PAYMENTS_PROVIDER=simulated`) + `StripePaymentProvider` stub swap-ready (webhook `POST /v1/billing/webhooks/stripe` `@Public` + raw-body signature, dedup table lands with swap; SaaS side = plain Stripe Billing, student side = Connect destination charges per db-05); charge generation = idempotent on-read materialization (`ON CONFLICT DO NOTHING` on db-05's unique key, same pass flips overdue) + audited admin trigger `POST /v1/admin/billing/charges/materialize`, real cron deferred to infra map; events `billing.charge.created/paid/overdue/refunded` → listener-only notifications (overdue = cobrança automática with Pix deep link); repasses read-model `GET /v1/platform/billing/repasses` (owner/finance, gross − fee_bps, withheld when delinquent)** — [issues/05-stripe-integration-architecture.md](issues/05-stripe-integration-architecture.md)
...
- ~~Realtime for the professor live-attendance screen~~ — resolved in `issues/09-realtime-live-attendance.md` (SSE + signed ticket + polling fallback).
