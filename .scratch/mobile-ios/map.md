...
- **02 networking**: apple/swift-openapi-generator as SPM build plugin in TatameAPI (generated code never committed; the committed cross-world artifact is the spec copy `Sources/TatameAPI/openapi.json`, synced from `packages/api-contract/openapi.json` as reviewed diffs), URLSession transport, tag-filtered to auth/public/aluno/professor/responsavel; Alamofire rejected; generated Client wrapped behind repository protocols declared in TatameCore; AuthMiddleware (ClientMiddleware) with retry-once-on-401 + single-flight `TokenRefreshCoordinator` actor, access token in memory, refresh token in Keychain (`kSecAttrAccessibleWhenUnlockedThisDeviceOnly`); problem+json → one typed `ApiError` enum branching on status + stable `code`; test seam = `ClientTransport` mock (no URLProtocol) → `issues/02-networking-api-client.md`
- **06 architecture**: MV with `@MainActor @Observable` feature models (model only where logic warrants; no ObservableObject, no per-screen-ViewModel dogma); TCA rejected (only third-party dep, no state-machine need in a parity port); one `@Observable` router per persona shell (tab enum + typed `NavigationStack` path per tab + sheet enums, deep-link-ready value destinations); server state via minimal `Loadable<Value>` enum in TatameCore + per-repository session caches, no cache framework; DI = repository protocols in TatameCore, concretes in TatameAPI, injected via `@Entry` environment at composition root, models take deps via `init` (test seam); async work in models via `.task`/intent methods, structured concurrency only → `issues/06-architecture-pattern.md`
